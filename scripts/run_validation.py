...
for exp in experiments:
    ...
    print(f"Running: {COUNTRY_LABEL} | {TARGET_NAME} | H={H}")
    ...
    if COUNTRY_LABEL == "India":
        N_SPLITS = int(len(X) * 0.2)
        N_SPLITS = min(max(N_SPLITS, 20), 30)
    else:
        N_SPLITS = int(len(X) * 0.25)
        N_SPLITS = min(max(N_SPLITS, 25), 40)

    if H > 1:
        N_SPLITS = 24 if COUNTRY_LABEL == "India" else 35

    tscv = TimeSeriesSplit(n_splits=N_SPLITS, test_size=1)

    predictions = []
    actuals = []
    model_names = ['Naive', 'SARIMA', 'SARIMAX', 'Lasso', 'RandomForest', 'XGBoost']
    errors = {name: [] for name in model_names}

    nested_tscv = TimeSeriesSplit(n_splits=5, test_size=1)
    pipe_lasso = Pipeline([('scaler', StandardScaler()), ('model', Lasso(random_state=RANDOM_STATE, max_iter=10000))])
    grid_lasso = {'model__alpha': [0.001, 0.01, 0.1, 1.0, 10.0]}
    pipe_rf = Pipeline([('scaler', StandardScaler()), ('model', RandomForestRegressor(random_state=RANDOM_STATE, n_jobs=-1))])
    grid_rf = {'model__n_estimators': [50, 100], 'model__max_depth': [3, 5]}
    pipe_xgb = Pipeline([('scaler', StandardScaler()), ('model', XGBRegressor(random_state=RANDOM_STATE, n_jobs=-1))])
    ...
    for i, (train_index, test_index) in enumerate(tscv.split(X)):
        ...
        fold_preds['Naive'] = pred_naive
        errors['Naive'].append(actual - pred_naive)
        ...
        try:
            ...
            pred_sarima = model_sarima.predict(n_periods=H)[-1]
        except:
            pred_sarima = 0.0
    ...
    for model_name in model_names:
        ...
        if model_name not in ['Naive', 'SARIMA']:
